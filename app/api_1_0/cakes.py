...
def get_cakes():
  ...
  return jsonify({'cakes' : [cake.to_json(cake.category.to_json()) for cake in cakes]})

@api.route('/cakes/<int:id>')
def get_cake(id):
  cake = Cake.query.get_or_404(id)
  return jsonify(cake.to_json(cake.category.to_json()))

@api.route('/cakes/<int:id>/orderNum')
def get_cake_orderNum(id):
  orderNum = Order.query.filter_by(cakeId=id).count()
...
def new_cake(id):
  ...
  db.session.add(cake)
  db.session.commit()
  return jsonify(cake.to_json(cake.category.to_json()), 201, {'Location' : url_for('api.get_cake', id=cake.id, _external=True)})
